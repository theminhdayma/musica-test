...
const visible = computed(() => (open.value ? props.items : props.items.slice(0, 9)))
</script>
...
<template>
  <div class="card section">
    ...
    <div class="grid">
      <button v-for="g in visible" :key="g.id" type="button" class="genre" @click="emit('select', g.id)">
...
.genre:hover .name { text-decoration: underline; }
.thumb { width: 34px; height: 34px; border-radius: 0; background: var(--c-bg); border: 1px solid var(--c-border); }

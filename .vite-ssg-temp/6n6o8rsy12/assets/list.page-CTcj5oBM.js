...
import { l as listMyCertificates } from "./api-CYbEatvG.js";
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const resource = useAsyncResource(async () => {
      return await listMyCertificates({ page: page.value, pageSize, q: q.value.trim() || void 0 });
    });
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "container section" }, _attrs))}><div style="${ssrRenderStyle({ "display": "flex", "gap": "12px", "align-items": "baseline", "justify-content": "space-between", "flex-wrap": "wrap" })}"><h1 style="${ssrRenderStyle({ "margin": "0" })}">Chứng nhận tác quyền của tôi</h1><form style="${ssrRenderStyle({ "display": "flex", "gap": "10px", "align-items": "center" })}"><input${ssrRenderAttr("value", q.value)} class="input" type="search" placeholder="Tìm theo mã/tên tác phẩm…" style="${ssrRenderStyle({ "min-width": "260px" })}"><button class="btn btn-soft btn-sm" type="submit">Tìm</button></form></div><div style="${ssrRenderStyle({ "margin-top": "18px", "display": "grid", "gap": "12px" })}">`);
      if (unref(resource).status.value === "loading" || unref(resource).status.value === "idle") {
        ...
        _push(`<div class="card"><div style="${ssrRenderStyle({ "font-weight": "800" })}">Chưa có chứng nhận</div><div class="muted" style="${ssrRenderStyle({ "margin-top": "6px" })}">Bạn sẽ thấy chứng nhận sau khi hoàn tất giao dịch.</div></div>`);
      } else {
        _push(`<!--[-->`);
        ssrRenderList(items.value, (it) => {
          _push(ssrRenderComponent(_component_RouterLink, {
            ...
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
                _push2(`<div style="${ssrRenderStyle({ "min-width": "0" })}"${_scopeId}><div style="${ssrRenderStyle({ "font-weight": "800", "white-space": "nowrap", "overflow": "hidden", "text-overflow": "ellipsis" })}"${_scopeId}>${ssrInterpolate(it.productTitle)}</div><div class="muted" style="${ssrRenderStyle({ "margin-top": "4px" })}"${_scopeId}>${ssrInterpolate(it.productCode)} · ${ssrInterpolate(it.artistDisplayName)}</div></div><div class="muted" style="${ssrRenderStyle({ "white-space": "nowrap" })}"${_scopeId}>${ssrInterpolate(new Date(it.createdAt).toLocaleDateString("vi-VN"))}</div>`);
              } else {
                return [
                  createVNode("div", { style: { "min-width": "0" } }, [
                    createVNode("div", { style: { "font-weight": "800", "white-space": "nowrap", "overflow": "hidden", "text-overflow": "ellipsis" } }, toDisplayString(it.productTitle), 1),
                    createVNode("div", {
                      class: "muted",
                      style: { "margin-top": "4px" }
                    }, toDisplayString(it.productCode) + " · " + toDisplayString(it.artistDisplayName), 1)
                  ]),
                  createVNode("div", {
                    class: "muted",
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("src/pages/certificates/list.page.vue");
